...
Tool costs and tool-effect consumption now share one rule interface; non-tool consumption has its own entry. Recovery tongs, Toast, selection lists and bin badges use the same eligibility rule. The bin shows consumption sources, animations distinguish tool consumption from other effects, and rule descriptions emphasize their target objects. Copy selection shows enchantments, closing-meal selection preserves scrolling, and table grouping animates.

Existing saves and original art are retained. The growth test-mode entry remains hidden. 312 automated checks pass, with desktop and mobile visual checks.
