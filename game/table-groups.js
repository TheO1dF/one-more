...
export function settledHTML(s,groups,lang='zh',busy=false){
 if(!groups.enabled)return '';
 const en=lang==='en',paired=groups.eligible.filter(c=>c.pair),spent=groups.eligible.filter(c=>!c.pair);
 return `<div class="settled-tray"><button data-action="table-expand" aria-expanded="${groups.expanded}" ${busy?'disabled':''}>
 <span class="settled-icons" aria-hidden="true">${groups.eligible.slice(0,3).map(c=>icon(c.kind)).join('')}</span>
 <span>${paired.length?`${en?'Paired':'已配对'} ${paired.length} · ${points(paired.reduce((n,c)=>n+value(s,c),0))}${en?' pts':'分'}`:''}${paired.length&&spent.length?' / ':''}${spent.length?`${en?'Spent tools':'已横置'} ${spent.length}`:''}</span>
 <b>${groups.expanded?(en?'COLLAPSE':'收拢'):(en?'EXPAND':'展开')}</b></button></div>`;
}
