...
export function enchantmentHTML(card,lang='zh'){
 const e=ENCHANTMENTS[card.enchantment];if(!e)return '';
 const i=lang==='en'?1:0,effect=enchantmentText(card,card.enchantment,CARDS)||e.text;
 return `<span class="card-enchantment" data-enchantment="${card.enchantment}"><b class="card-enchantment-label">${esc(e.name[i])}</b><span class="card-enchantment-effect">${esc(effect[i])}</span></span>`;
}
export function enchantedName(card,lang='zh'){
 const e=ENCHANTMENTS[card.enchantment];
 return nameOf(card.original||card.kind,lang)+(e?` · ${e.name[lang==='en'?1:0]}`:'');
}
