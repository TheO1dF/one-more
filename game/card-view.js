...
export function materialLayers(enchantment){
 return MATERIALS[enchantment]?'<span class="card-material" aria-hidden="true"><i class="material-grain"></i><i class="material-sheen"></i><i class="material-motes"></i></span>':'';
}
export function cardHTML(c,{s,lang='zh',selected,flow,performance}={}){
 ...
 return `<div class="card-seat ${c.tapped?'landscape':''}" style="--offset:${c.uid*13%7-3}px"><button class="tile ${type} ${selected===c.uid?'selected':''} ${c.pair?'paired':''} ${c.tapped?'tapped':''} ${c.sealedBy?'sealed':''} ${target?'targetable':''} ${material?.className||''}" data-action="select" data-uid="${c.uid}" data-kind="${c.kind}" data-material="${c.enchantment||'paper'}" data-angle="${angle}" style="--tilt:${angle}deg;--material-phase:${-(c.uid%7)}s;${performance==='draw'&&c.uid===selected?'visibility:hidden;':''}" aria-label="${esc(nameOf(c.kind,lang)+' · '+badge+(material?' · '+tx(material.label):''))}" aria-pressed="${selected===c.uid}"><span class="tile-top"><span>${types[type]}</span>${points?`<b>${c.kind==='cola'?'Σ'+onTable(s).filter(x=>x.kind==='cola').reduce((n,x)=>n+value(s,x),0):value(s,c)}</b>`:''}</span>${icon(c.kind)}${materialLayers(c.enchantment)}${material?`<span class="enchant-stamp ${c.enchantment}" title="${esc(tx(ENCHANTMENTS[c.enchantment].text))}">${tx(ENCHANTMENTS[c.enchantment].stamp)}</span>`:''}<strong>${nameOf(c.kind,lang)}</strong><span class="tile-badge ${matches?'match':''}">${badge}</span></button></div>`;
}
