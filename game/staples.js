...
export function closedStaple(s,uid){
 ...
 return b&&b.readyRound<=s.round&&b.uids.every(id=>s.draw.includes(id))?b:null;
}
...
export function unfasten(s,uid){
 const b=stapleFor(s,uid);if(b)s.staples=s.staples.filter(x=>x.id!==b.id);return b;
}
export function validStaples(s){
 if(s.staples==null)return true;
 if(!s.growth||!Array.isArray(s.staples)||!Number.isSafeInteger(s.stapleId)||s.stapleId<0)return false;
 ...
 for(const b of s.staples){
  ...
  if(b.readyRound<=s.round){const at=s.draw.indexOf(b.uids[0]);if(at<0||b.uids.some((uid,i)=>s.draw[at+i]!==uid))return false;}
 }
 return true;
}
