...
export function posterBack(variant='casino'){
 if(variant!=='casino'){
 const bg=variant==='ivory'?P.cream:variant==='midnight'?P.ink:P.orange;
 const fg=variant==='ivory'?P.purple:variant==='midnight'?P.pink:P.cream;
 const shape=variant==='roulette'?Array.from({length:12},(_,i)=>'<path d="M56 29h6v20h-6Z" transform="rotate('+i*30+' 59 84)" fill="'+fg+'"/>').join('')+'<circle cx="59" cy="84" r="27" fill="'+P.ink+'"/><circle cx="59" cy="84" r="17" fill="'+P.yellow+'"/>':variant==='midnight'?'<path d="M20 131 58 31l40 100-39-25Z" fill="'+fg+'"/><path d="M59 59 72 92H46Z" fill="'+P.cream+'"/>':Array.from({length:5},(_,i)=>'<path d="m59 '+(27+i*21)+' 18 13-18 13-18-13Z" fill="'+fg+'"/>').join('');
 return '<svg class="back-art" viewBox="0 0 118 168" aria-hidden="true"><rect width="118" height="168" rx="5" fill="'+P.cream+'"/><path d="M7 7h104v154H7Z" fill="'+bg+'"/>'+shape+'</svg>';
 }
