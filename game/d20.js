...
export function drawD20(canvas,{value=20,spin=[0,0,0],x=.5,y=.5,size=62,lift=0,shadow=true}={}){
 ...
 if(shadow){ctx.fillStyle='#161936';ctx.beginPath();ctx.ellipse(cx,cy+size*.78,size*(.65+lift*.18),size*.19,0,0,Math.PI*2);ctx.fill();}
 ...
 for(const f of visible){const p=f.ids.map(i=>project(vs[i]));ctx.beginPath();ctx.moveTo(...p[0]);ctx.lineTo(...p[1]);ctx.lineTo(...p[2]);ctx.closePath();const light=Math.max(0,dot(f.n,norm([-.6,.9,1.1])));ctx.fillStyle=light>.78?'#fff8e8':light>.54?'#edbd38':light>.28?'#ff4928':'#574798';ctx.fill();ctx.strokeStyle='#161936';ctx.lineWidth=2;ctx.stroke();
