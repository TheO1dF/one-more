export function arrangeCards(cards, width, height) {
...
export function layoutTable({page=0,focusUid=null,lang='zh'}={}){
  ...
  const bounds=field.getBoundingClientRect(), layout=arrangeCards(cards,Math.max(80,bounds.width-24),Math.max(70,bounds.height-8));
  ...
  if(pager)pager.innerHTML=layout.pages.length>1?layout.pages.map((rows,i)=>`<button data-action="table-page" data-page="${i}" class="${i===page?'current':''}" aria-label="${lang==='en'?'Page':'第'} ${i+1}${lang==='en'?'':'页'}" aria-current="${i===page?'page':'false'}">${i+1}</button>`).join(''):'';
