...
test('practice chain: scrap completes fermentation, wild pairs with mint, flashlight readies and peeks', () => {
  ...
  s = act(s, { type: 'pair', ids: [1, 4], target: 2 }); assert.equal(score(s), 4); assert.equal(card(s, 2).tapped, false);
...
test('third matching food neither triples nor reuses an already paired card; wildcard uses partner ability once', () => {
  let s = fixture(['fish', 'fish', 'fish', 'wild']); s = act(s, { type: 'pair', ids: [1, 2] }); assert.equal(score(s), 6); assert.equal(s.known.length, 1);
  assert.throws(() => act(s, { type: 'pair', ids: [1, 3] }));
  s = act(s, { type: 'pair', ids: [3, 4] }); assert.equal(score(s), 8); assert.equal(s.log.filter(e => e.key === 'peek').length, 2);
});
...
test('oil prevents tool activation, but a rice pair clears oil and releases a normal action window', () => {
  ...
  s = act(s, { type: 'pair', ids: [1, 2], target: 3 }); s = act(s, { type: 'use', uid: 4 }); assert.equal(s.known.length, 1); assert.equal(score(s), 4);
});
...
test('wrap captures food, sealing its source releases food immediately, and fermentation is temporary', () => {
  ...
  s = act(s, { type: 'use', uid: 2, target: 1 }); assert.equal(card(s, rice).sealedBy, null); assert.equal(score(s), 1);
...
test('multiple wraps compete only for the immediate next card and never reinterpret bomb', () => {
  ...
  s = act(s, { type: 'draw' }); assert.equal(card(s, 2).caught, null); s = act(s, { type: 'draw' }); assert.equal(s.reason, 'bomb');
});

test('debt suppresses unpaired points, but food remains usable for costs', () => {
  let s = fixture(['rice', 'rice', 'debt', 'scope']); assert.equal(score(s), 0); s = act(s, { type: 'pair', ids: [1, 2] }); assert.equal(score(s), 4);
...
test('fog limits future peeks without blocking swaps or erasing earlier knowledge', () => {
  let s = fixture(['rice', 'scope', 'sorter', 'fog'], ['fish', 'mint', 'bomb']); s.known = [5, 6];
  s = act(s, { type: 'use', uid: 2, food: 1 }); assert.deepEqual(s.known, [5, 6]); assert.deepEqual(act(s, { type: 'use', uid: 3, ids: [5, 6] }).draw, [6,5,7]);
});
...
test('bell readies another tool for a food cost; cannot self-ready', () => {
  ...
  s = act(s, { type: 'use', uid: 4, food: 5, target: 3 }); assert.equal(card(s, 3).tapped, false); assert.equal(card(s, 4).tapped, true); assert.equal(s.log.filter(e => e.key === 'pair').length, 1); assert.equal(score(s), 4);
});
...
test('splitter and recycler retain physical-card history; food can be recovered only once per round', () => {
  ...
  s = act(s, { type: 'pair', ids: [1, 2] }); s = act(s, { type: 'relic', id: 'splitter', uid: 1 }); assert.equal(score(s), 2); assert.equal(partners(s, 1).length, 0);
...
test('lunchbox forgoes income, removes the carried card from next draw pile, and does not consume safe first reveal', () => {
  ...
  s = act(s, { type: 'stop', carry: 1 }); assert.equal(s.roundEarned, 1); s = act(settle(s), { type: 'next' });
...
test('cash-out commits exactly once and terminal results distinguish victory, shortfall, and bomb', () => {
  let s = fixture(['fish', 'fish']); s.round = 5; s.bank = 2; s.target = 6; s = act(s, { type: 'pair', ids: [1, 2] }); s = act(s, { type: 'stop' }); assert.equal(s.phase, 'won'); assert.equal(s.bank, 6); assert.throws(() => act(s, { type: 'stop' }));
