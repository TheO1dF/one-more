...
const t=await connectSmokeTransport(9227),out='.artifacts/smoke-one-more-v060';await mkdir(out,{recursive:true});
...
try{
 ...
 for(const [width,height,lang] of [[1280,800,'zh'],[390,844,'en']]){
  ...
  await click('#stop');await check(lang+' bank preserves tutorial score',(await state()).bank===10);await audit(lang+' lesson die',lang);await shot(lang+'-tutorial-die');
  ...
  await ev("localStorage.setItem('one-more.player.v1',JSON.stringify({storySeen:true,tutorialComplete:false}))");await t.send('Page.reload',{ignoreCache:true});await wait(160);await click('[data-action="new"]');await click('[data-action="skip-lesson"]');await check(lang+' skip tutorial starts real game',!(await state()).lesson&&await ev('!document.querySelector(".lesson")'));
 }
