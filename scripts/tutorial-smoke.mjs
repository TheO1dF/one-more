...
export async function runTutorialSmoke({send,root,width=390,height=844,lang='en',replay=false}){
 const out=root+'/.artifacts/smoke-one-more-v'+VERSION.replaceAll('.','');await mkdir(out,{recursive:true});const checks=[],steps=new Set();
 ...
 if(!replay)await ev(`localStorage.setItem('one-more.run.v5',${JSON.stringify(JSON.stringify(tutorialRun(7711)))});localStorage.setItem('one-more.preferences.v2',JSON.stringify({lang:${JSON.stringify(lang)},motion:false,sound:false,music:false}));`);
 await send('Page.reload');await wait(90);
 const beforeReplay=await ev("[localStorage.getItem('one-more.run.v5'),localStorage.getItem('one-more.player.v1')]");
 if(replay){await ev("document.querySelector('[data-action=settings]').click();document.querySelector('[data-action=replay-tutorial]').click()");}
 ...
 for(let i=0;i<50;i++){
  ...
  const step=await ev("document.querySelector('.lesson')?Number(document.querySelector('.lesson').dataset.step):null");if(step==null)break;steps.add(step);
 ...
 if(steps.size!==20||(!replay&&(result.lesson!=null||result.round!==2)))throw Error('Tutorial did not complete: '+JSON.stringify({steps:[...steps],round:result.round,lesson:result.lesson}));
 if(replay){const after=await ev("[localStorage.getItem('one-more.run.v5'),localStorage.getItem('one-more.player.v1')]");if(JSON.stringify(after)!==JSON.stringify(beforeReplay))throw Error('Replay overwrote original progress');if(!await ev("!!document.querySelector('.main-menu')"))throw Error('Replay did not return to menu');}
 const report={width,height,lang,replay,steps:[...steps],checks,round:result.round,phase:result.phase};await writeFile(out+`/tutorial-${width}-${lang}.json`,JSON.stringify(report,null,2));return report;
}
